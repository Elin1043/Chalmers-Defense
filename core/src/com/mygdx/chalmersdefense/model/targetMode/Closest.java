...
final class Closest implements ITargetMode {
    ...
    public IVirus getCorrectVirus(List<IVirus> virusInRange, float towerX, float towerY) {
        ...
        double closestDistance = Calculate.distanceBetweenPoints(towerX, towerY, closestVirus.getX(), closestVirus.getY());

        for (IVirus virus : virusInRange) {
            double rangeToVirus = Calculate.distanceBetweenPoints(towerX, towerY, virus.getX(), virus.getY());
